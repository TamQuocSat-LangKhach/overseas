...
Fk:loadTranslationTable{
  ['os__piankuang'] = '偏狂',
  [':os__piankuang'] = '锁定技，当你使用【杀】对目标角色造成伤害时，若你本回合使用【杀】造成过伤害，此伤害+1。你的回合内，当你使用【杀】结算后，若此【杀】未造成伤害，本回合你手牌上限-1。',
...
piankuang:addEffect(fk.DamageCaused, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    player:broadcastSkillInvoke(piankuang.name)
    room:notifySkillInvoked(player, piankuang.name, "offensive")
...
piankuang:addEffect(fk.CardUseFinished, {
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(piankuang.name) and data.card and data.card.trueName == "slash" then
      ...
      return turn_event and turn_event.data[1] == player and not data.damageDealt
    end
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    player:broadcastSkillInvoke(piankuang.name)
    room:notifySkillInvoked(player, piankuang.name, "negative")

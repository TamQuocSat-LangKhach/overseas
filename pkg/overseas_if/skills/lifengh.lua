...
Fk:loadTranslationTable{
  ...
  [':os__lifengh'] = '出牌阶段，你可以弃置两张点数不同的牌，对一名距离X以内的角色造成1点伤害（X为这两张牌点数之差），其受到此伤害时，可以重铸一张手牌（没有手牌则改为摸一张牌），若此牌点数介于这两张牌点数闭区间，防止此伤害且本回合此技能失效。',
...
lifengh:addEffect('active', {
  ...
  target_filter = function (self, player, to_select, selected, selected_cards)
    if #selected_cards == 2 then
      return player:distanceTo(player.room:getPlayerById(to_select)) <=
...
lifengh:addEffect(fk.DamageInflicted, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    table.sort(nums)
    local card = {}
    if player:isKongcheng() and room:askToSkillInvoke(player, {skill_name = "os__lifengh", prompt = "#os__lifengh-draw:"..src.id..":"..nums[1]..":"..nums[2]}) then
